/**
 *  Smart Alarm (Virtual Alarm Panel) is a multi-zone virtual alarm panel, featuring customizable
...
definition(
 ...
 simply by setting SmartThings location 'mode'.''',
    ...
    oauth: [displayName:"Virtual Alarm Panel", displayLink:"http://statusbits.github.io/smartalarm/"]
)
...
def pageUninstall() {
    ...
    def text =
        "Virtual Alarm Panel version ${getVersion()} is not backward compatible " +
        "with the currently installed version. Please uninstall the " +
        "current version by tapping the Uninstall button below, then " +
        "re-install Virtual Alarm Panel from the Dashboard. We are sorry for the " +
        "inconvenience."
...
def pageArmingOptions() {
    ...
    def helpArming =
        "Virtual Alarm Panel can be armed and disarmed by setting the home Mode. " +
...
def pageAlarmOptions() {
    ...
    def helpAlarm =
        "You can configure Virtual Alarm Panel to take several actions when an " +
...
def pageRestApiOptions() {
    ...
    def textHelp =
        "Virtual Alarm Panel can be controlled remotely by any Web client using " +
        "REST API. Please refer to Virtual Alarm Panel documentation for more " +
        "information."

    def textPincode =
        "You can specify optional PIN code to protect arming and disarming " +
        "Virtual Alarm Panel via REST API from unauthorized access. If set, the " +
